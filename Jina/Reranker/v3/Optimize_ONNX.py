...
original_folder_path = os.path.join(SCRIPT_DIR, 'Reranker_ONNX')
quanted_folder_path = os.path.join(SCRIPT_DIR, 'Reranker_Optimized')
...
# ==============================================================================
MODEL_DTYPE = {
    "Reranker_Embed":  "int4",       # [int4, float32, float16]
    "Reranker_Main":   "int4",       # [int4, int8, float32, float16]
    "Reranker_Concat": "float32",    # [float32, float16]
    "Reranker_Score":  "float32",    # [float32, float16]
}
...
def _is_transformer_block(name):
    return name in ("Reranker_Main")


def _is_embed_block(name):
    return name in ("Reranker_Embed")
...
def read_metadata_props(onnx_path):
    """Return {key: value} of an ONNX model's custom metadata_props.

    Loaded WITHOUT external data so only the graph proto is parsed; the dict is
    extracted and the model freed immediately, so this transient read never
    overlaps the heavy quant / optimize passes that follow.  Returns {} if the
    model cannot be read.
    """
    try:
        model = onnx.load(onnx_path, load_external_data=False)
    except Exception as exc:
        print(f"  [metadata] Could not read metadata from {onnx_path}: {exc}")
        return {}
...
def apply_metadata_props(model, metadata):
    """Add key/value metadata_props onto a model (skips keys already present).

    The optimize / quant / slim passes build fresh graphs that drop the source
    metadata_props, so the captured source metadata is re-applied to the final
    output — Reranker_Score carries the geometry the inference reads.
    """
...
def process_single_model(
    ...
    # Capture the source model's custom metadata up-front; the passes below
    # rebuild the graph and drop metadata_props, so it is re-applied to the
    # final saved model (Reranker_Score carries the geometry the inference
    # reads from the ONNX metadata).
    src_metadata = read_metadata_props(model_path)
    ...
    if upgrade_opset > 0:
        ...
        try:
            ...
            apply_metadata_props(converted_model, src_metadata)
            ...
            m = onnx.load(quanted_model_path)
            apply_metadata_props(m, src_metadata)
